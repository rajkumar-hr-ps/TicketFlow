...
**Version:** 4.0.0
**Date:** 2026-02-17
**Runtime:** Node.js (Express + MongoDB + Mongoose + Redis + BullMQ)
...
- Payment is processed asynchronously via BullMQ job queue
...
**Async Processing:**
- Payment processing via BullMQ job queue (Redis-backed)
- Hold expiry cleanup via scheduled BullMQ jobs
- Waitlist notification via BullMQ job queue
...
| **ODM** | Mongoose | 9.x+ |
| **Cache / Locks** | Redis | 8.x+ |
| **Job Queue** | BullMQ | 5.x+ |
| **Authentication** | JWT (jsonwebtoken) | HS256 |
| **Password Hashing** | bcryptjs | — |
| **Testing** | Mocha + Chai + Supertest | Mocha 11.x, Chai 5.x |
...
src/
  ...
  jobs/
    payment.processor.js              # BullMQ job: process payments
    holdExpiry.processor.js           # BullMQ job: cleanup expired holds
    waitlistNotifier.processor.js     # BullMQ job: notify waitlist
...
When a ticket becomes available (via refund or cancellation), the system notifies the next person on the waitlist via a BullMQ job.
