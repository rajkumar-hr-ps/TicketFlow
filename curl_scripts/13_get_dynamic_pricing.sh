...
if [ -z "$EVENT_ID" ] || [ -z "$VENUE_SECTION_ID" ]; then
  echo "Error: EVENT_ID and VENUE_SECTION_ID must be set"
...
RESPONSE=$(curl -s -X GET "${BASE_URL}/api/v1/events/${EVENT_ID}/pricing?section_id=${VENUE_SECTION_ID}&quantity=${QUANTITY}" \
